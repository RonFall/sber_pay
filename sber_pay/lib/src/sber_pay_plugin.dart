import 'package:sber_pay/src/sber_pay_payment_status.dart';
...
class SberPayPlugin {
  ...
  static Future<bool> initSberPay({
    ...
    final result = await _platform.initSberPay(
      env: env,
  ...
  static Future<bool> isReadyForSPaySdk() async {
    final result = await _platform.isReadyForSPaySdk();
    if (result == null || result == false) {
      await Future.delayed(
        const Duration(seconds: 2),
        () async {
          return await _platform.isReadyForSPaySdk();
        },
      );
    }
    return result ?? false;
  ...
  static Future<SberPayPaymentStatus> payWithBankInvoiceId({
    ...
    final result = await _platform.payWithBankInvoiceId(
      apiKey: apiKey,
