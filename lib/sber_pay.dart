...
class SberPayPlugin {
  ...
  /// Инициализация SberPay SDK. Для работы с библиотекой сначала вызывается
  /// эта функция. На стороне Android этот метод является асинхронным, однако у
  ...
  /// Метод для проверки готовности к оплате. Зависит от переданного аргумента
  /// [env] при инициализации через метод [initSberPay] (см. комментарий к
  /// методу).
  ///
  ...
  ///
  /// [cancel] - Отмена платежа (только на iOS).
  ///
  /// в остальных случаях - текст ошибки.
